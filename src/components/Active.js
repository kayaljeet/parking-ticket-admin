...
export default function Active() {
  ...
  const handleUpdate = async () => {
    ...
    if (updatedData.assigned_to && updatedData.challan_amount) {
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                id="assigned_to"
                value={selectedChallan?.assigned_to || ''}
                onChange={(e) => handleInputChange('assigned_to', e.target.value)}
                className="col-span-3"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <label htmlFor="challan_amount" className="text-right">
                Challan Amount
              </label>
              <Input
                id="challan_amount"
                type="number"
                value={selectedChallan?.challan_amount || ''}
                onChange={(e) => handleInputChange('challan_amount', e.target.value)}
                className="col-span-3"
              />
            </div>
          </div>
