...
async function startServer() {
  try {
    ...
    app.get('/locations', async (req, res) => {
      try {
        const challans = await collection.find({ status: { $in: ['Pending', 'Active'] } }).toArray();
        console.log(`Fetched ${challans.length} challans with status 'Pending' or 'Active'`);
        res.json(challans);
      } catch (error) {
    ...
    app.get('/completed', async (req, res) => {
      try {
        const challans = await collection.find({ status: 'Completed' }).toArray();
        console.log(`Fetched ${challans.length} challans with status 'Completed'`);
        res.json(challans);
      } catch (error) {
        console.error('Error fetching challans:', error);
        res.status(500).send('Error fetching challans');
